...
namespace DAL
{
    ...
    public class dalComment
    {
        ...
        {
            string sql = "insert into Comment(hotelObj,content,userObj,commentTime) values(@hotelObj,@content,@userObj,@commentTime)";
            /*构建sql参数*/
            ...
            /*给参数赋值*/
            parm[0].Value = comment.hotelObj; //被评酒店
        ...
        public static ENTITY.Comment getSomeComment(int commentId)
        {
            ...
            if (DataRead.Read())
            {
                ...
                comment.hotelObj = Convert.ToInt32(DataRead["hotelObj"]);
                comment.content = DataRead["content"].ToString();
                comment.userObj = DataRead["userObj"].ToString();
                comment.commentTime = Convert.ToDateTime(DataRead["commentTime"].ToString());
            }
            return comment;
        }
        ...
        {
            string sql = "update Comment set hotelObj=@hotelObj,content=@content,userObj=@userObj,commentTime=@commentTime where commentId=@commentId";
            /*构建sql参数信息*/
            SqlParameter[] parm = new SqlParameter[] {
             new SqlParameter("@hotelObj",SqlDbType.Int),
             new SqlParameter("@content",SqlDbType.VarChar),
